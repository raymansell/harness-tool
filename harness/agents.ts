...
// The generalist. The triage agent serves as the point of entry agent that takes every single request.
// If it can handle the request, it will do it. If it thinks another agent is better suited for the task
// or if it's not allowed to handle it, it will hand off to another agent.
// Note what it does NOT have: issueRefund. It can talk about a
// refund, but it isn't allowed to move money — that's the whole reason it hands off.
