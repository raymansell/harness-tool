...
// timeout can't interrupt async code. For a production system, run
...
export async function runInSandbox(
  ...
  // This is a workaround for top-level async/await.
  ...
  try {
    ...
    // The ok property provides a simple boolean indicator that an AI can easily understand to determine if code execution was successful (ok: true) or if an error occurred (ok: false). This explicit success/failure signal helps the AI interpret the execution results correctly.
...
function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  // Promise.race() is used to implement a timeout by racing two promises:
  // the actual code execution and a timer. Whichever promise resolves first
  // (either the code finishes or the timeout expires) determines the outcome,
  // ensuring code doesn't run indefinitely.
  return Promise.race([
    promise,
